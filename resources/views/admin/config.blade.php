...
@section('content')
  ...
  <div class="config-wrapper">
    <ul class="side-bar">
      ...
      <li class="bar-item"><a class="link" href="{{ route('admin.show', ['uid' => $admin->uid]) }}">戻る</a></li>
    ...
    <div id="confProf" class="config profile-config">
      ...
      <form class="form" action="{{ route('admin.update', ['uid' => $admin->uid]) }}" method="POST">
        @csrf
        <div class="row">
          <div class="col-2">電話番号</div>
          <div class="form-group col tel">
            <input class="input" name="tel_no" type="text" value="{{ $admin->tel_no }}">
          </div>
        </div>
        <div class="row">
          <div class="col-2">メールアドレス</div>
          <div class="form-group col email">
            <input class="input" name="email" type="text" value="{{ $admin->email }}">
          </div>
        </div>
